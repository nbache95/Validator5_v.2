...
include_once('../class/jsontocsv.class.php');
...
if(is_array($checkValued)){
    foreach ($checkValued as $valeur){
        ...
        if($valeur == 'checkCSS'){
            ...
            $valCss->setValUrlCss();
            echo($valCss);
			echo '<div><a href="./member.php?d_user='.$id_user.'">Retour à la page d\'accueil</a></div>';
        }
        if($valeur == 'checkhtml'){
            $valHtml = new ValidationHtml($url);
            $valHtml->setValUrlHtml();
            $csv = new jsonToCsv($valHtml);
...
//             echo '</pre><br>';
// 			echo '<div>
//                     <a href="./member.php?d_user='.$id_user.'">Retour à la page d\'accueil</a>
//                     <div class="">Télécharger le compte rendu csv</div>
//                   </div>';
        }
...
$db = new Database();
$db->connexion(); 
$sql = "INSERT INTO analysis (id_users , url_site) VALUES ('$id_user', '$url')"; 
$db->insertquery($sql); 
